...
function AddArtistAlbumButton({ refreshData }) {
  const [userFormData, setUserFormData] = useState({ artistID: null, albumID: null });
  ...
  const fetchData = async () => {
    try {
      ...
      const artist_default = artistsData[0] ? artistsData[0].artist_id : null;
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      fetch(crud_address + '/api/artist-album-details', {
        ...
        body: JSON.stringify({
          artist_id: userFormData.artistID,
  ...
  return (
    <>
      <Button variant="outline-primary" onClick={handleShow}>
        Add Artist-Album Details
      </Button>

      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Add Artist-Album Details</Modal.Title>
        </Modal.Header>
        <Modal.Body>
...
export default AddArtistAlbumButton;